...
public abstract class TileEntityMachine extends TileEntityInventoryAndTank {
	...
	/**
	 * Called every time the idle timer ticks. Use this for free things like filling/emptying buckets
	 */
	protected abstract void doPreWork();
	
	/**
	 * Called after PreWork if work is 0, check for work here and it will be added before calling work. 
	 */
	...
	public boolean wrenchedShift(EntityPlayer player, ForgeDirection side){
		incrementEjectMode();
		player.addChatMessage("Eject mode set to " + getEjectMode());
		return false;
	}
	...
	protected void ejectFluid(ForgeDirection side, AdvFluidTank tank, int maxDrain){
		...
		if(otherTile != null && IFluidHandler.class.isAssignableFrom(otherTile.getClass()) && (currMode == ejectMode.ALL || TileEntityMachine.class.isAssignableFrom(otherTile.getClass()))){//IFluidHandler
			FluidStack drain = tank.drain(side, maxDrain, false);
			if(drain == null)
				return;
