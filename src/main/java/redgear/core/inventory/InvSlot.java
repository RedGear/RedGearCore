...
/**
 * Different from <{@link Slot}>, this class is used to store and sort the stored ItemStacks of an inventory, as well as 
...
public class InvSlot extends Slot {
	...
	public boolean canAddStack(ItemStack stack, boolean all, boolean override){
		...
		return contents.isStackable() && stack.isItemEqual(contents) && ItemStack.areItemStackTagsEqual(stack,  contents) && stackAllowed(stack) && contents.stackSize < contents.getMaxStackSize() && (override || pressure <= 0) && (!all || contents.stackSize + stack.stackSize <= contents.getMaxStackSize());
	}
